...
def checkRegEx(regex):
	if regex[0] == '*' or regex[0] == '+' or regex[0] == '.':
		return False
	if regex[len(regex)-1] == '+' or regex[len(regex)-1] == '.':
		return False
	stack = []
	l =len(regex)
...
def getStarHeight(regex):
	sh = 0
	int(sh)
	flag = 0
	int(flag)
	for i in range(0, len(regex)):
		if regex[i] == '*' and flag == 0:
			sh = sh + 1
			flag = 1
		elif regex[i] == ')':
			flag = 0
...
class AutomataGUI:
    ...
    def initUI(self):
        ...
        self.bottomLabel = Label(parentFrame, text="Aditya Sharma Dept. of CSE, CCET")
    ...
    def handleBuildRegexButton(self):
        ...
        try:
            ...
            if inp == '':
                ...
                return
            if checkRegEx(inp):
            	sh = getStarHeight(inp)
            	self.statusLabel.config(text="Valid Regular Expression! Star height = %d!" % sh)
            	self.createAutomata(inp)
            else:
            	self.statusLabel.config(text="Invalid Regular Expression!")
